...
export default function TableOfContents({ contentId }: { contentId: string }) {
  ...
  return (
    <nav className="sticky top-16">
      <h2 className="text-sm/6 font-semibold">On this page</h2>
      <ul className="mt-3 flex flex-col gap-3 border-l text-sm/6 border-white/10">
        {headings.map((heading, i) => (
          <li
            key={i}
            className={clsx(
              "-ml-px border-l border-transparent pl-4",
              "hover:not-has-aria-[current=location]:border-gray-400 hover:text-white",
              "has-aria-[current=location]:border-white"
            )}
          >
            <a
              href={`#${heading.id}`}
              ...
              className={clsx(
                ...
                "block aria-[current=location]:font-medium aria-[current=location]:text-white"
              )}
            >
              {heading.text}
            </a>
